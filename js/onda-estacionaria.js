...
(function () {
  ...
  // C Estado editable: armónico, velocidad de onda, longitud de cuerda y tiempo.
  let n = 3, v = 50, L_cuerda = 10, t = 0;
  let running = false, raf = null;

  // F En cuerda fija, fₙ = n·v/(2L) y λₙ = 2L/n.
  const fn = () => n * (v / (2 * L_cuerda));
  const w = () => 2 * Math.PI * fn();
  const k = () => (n * Math.PI) / L_cuerda;
  const lambda = () => 2 * L_cuerda / n;

  // C Dibuja nodos, envolvente y forma instantánea de la onda estacionaria.
  function draw() {
    const W = cv.width, H = cv.height, eqY = H / 2, padding = 30;
    const drawW = W - padding * 2;
    bgd(ctx, W, H);

    // C Eje X y bordes fijos (Nodos externos)
  ...
  // F Actualiza frecuencia modal, longitud de onda y número de nodos.
  function updCalc() {
  ...
  // C Anima la oscilación temporal del modo normal seleccionado.
  function loop() { if (!running) return; t += 0.016; draw(); raf = requestAnimationFrame(loop); }
  // C Ajusta el lienzo y pinta el modo normal inicial.
